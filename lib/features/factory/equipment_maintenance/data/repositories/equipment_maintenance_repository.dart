...
class MockEquipmentMaintenanceRepository
    extends EquipmentMaintenanceRepository {
  final MockDataService mockDataService;
  MockEquipmentMaintenanceRepository(this.mockDataService) : super();

  @override
  Future<List<EquipmentModel>> getAllEquipment() async {
    return mockDataService.getAllMockEquipment();
  }

  @override
  Future<EquipmentModel> getEquipmentById(String id) async {
    final eq = mockDataService.getMockEquipmentById(id);
    if (eq == null) throw Exception('Equipment not found');
    return eq;
  }

  @override
  Future<String> createEquipment(EquipmentModel equipment) async {
    final newId = 'eq-mock-${DateTime.now().millisecondsSinceEpoch}';
    final newEquipment = equipment.copyWith(id: newId);
    mockDataService.addMockEquipment(newEquipment);
    return newId;
  }

  @override
  Future<void> updateEquipment(EquipmentModel equipment) async {
    mockDataService.updateMockEquipment(equipment);
  }

  @override
  Future<void> updateEquipmentStatus(
      {required String equipmentId, required EquipmentStatus status}) async {
    final eq = mockDataService.getMockEquipmentById(equipmentId);
    if (eq == null) throw Exception('Equipment not found');
    mockDataService.updateMockEquipment(eq.copyWith(status: status));
  }
  ...
  Future<List<EquipmentModel>> getEquipmentRequiringMaintenance() async {
    ...
    return mockDataService
        ...
        .where((e) =>
            ...
            e.nextMaintenanceDate!.isBefore(now))
        .toList();
  }
  ...
  Future<List<EquipmentModel>> getEquipmentRequiringSanitization() async {
    final now = DateTime.now();
    return mockDataService.getAllMockEquipment().where((e) {
      if (!e.requiresSanitization || e.lastSanitizationDate == null)
        return false;
      final nextSanitization = e.lastSanitizationDate!
          .add(Duration(hours: e.sanitizationIntervalHours));
      return nextSanitization.isBefore(now);
    }).toList();
  }
  ...
  Future<List<MaintenanceRecordModel>> getMaintenanceRecordsForEquipment(
      String equipmentId) async {
    return mockDataService.getMockMaintenanceRecordsForEquipment(equipmentId);
  }
  ...
  @override
  Future<MaintenanceRecordModel> getMaintenanceRecordById(String id) async {
    final rec = mockDataService.getMockMaintenanceRecordById(id);
    if (rec == null) throw Exception('Maintenance record not found');
    return rec;
  }

  @override
  Future<String> createMaintenanceRecord(MaintenanceRecordModel record) async {
    final newId = 'mr-mock-${DateTime.now().millisecondsSinceEpoch}';
    final newRecord = record.copyWith(id: newId);
    mockDataService.addMockMaintenanceRecord(newRecord);
    return newId;
  }

  @override
  Future<void> updateMaintenanceRecord(MaintenanceRecordModel record) async {
    mockDataService.updateMockMaintenanceRecord(record);
  }

  @override
  Future<List<String>> createScheduledMaintenanceForAll() async {
    // For mock, just return empty
    return [];
  }
}

final mockEquipmentMaintenanceRepositoryProvider =
    Provider<MockEquipmentMaintenanceRepository>((ref) {
  final mockDataService = ref.read(mockDataServiceProvider);
  return MockEquipmentMaintenanceRepository(mockDataService);
});

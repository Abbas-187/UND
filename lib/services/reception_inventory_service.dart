...
import '../features/inventory/data/models/batch_information.dart';
...
/// Define a local class for quality parameter measurement since it is missing in the codebase
class QualityParameterMeasurement {
  QualityParameterMeasurement({
...
class ReceptionInventoryService {
  ...
  Future<String> createInventoryFromReception(String receptionId) async {
    try {
      ...
      final inventoryItem = InventoryItemModel(
        ...
        appItemId: 'MILK-$lotNumber', // Generating a unique app item ID
        name: itemName,
        category: category,
        unit: 'liters',
        ...
        additionalAttributes: {
          ...
          // Store extra info as JSON-serializable maps/lists
          'temperatureHistory': [
            {
              'timestamp': reception.timestamp.toIso8601String(),
          ...
          'qualityParameters': qualityParams.map((k, v) => MapEntry(k, {
          ...
          'batchInformation': {
            ...
            'productionDate': batchInfo.productionDate.toIso8601String(),
            'sourceId': batchInfo.sourceId,
            'sourceDetails': batchInfo.sourceDetails,
            'isApproved': batchInfo.isApproved,
            'approvalDate': batchInfo.approvalDate.toIso8601String(),
          ...
          'overallQualityStatus':
              _determineOverallQualityStatus(reception).toString(),
        },
      );
  ...
  Future<void> _triggerLowInventoryAlert(String itemName) async {
    try {
      ...
      debugPrint('Failed to trigger low inventory alert: $e');
    }
  }
  ...
  Future<void> _updateInventoryAnalytics(
    ...
    try {
      ...
      // Log error but don't throw
      debugPrint('Failed to update inventory analytics: $e');
    }
  }
  ...
  Future<void> _triggerTemperatureAlert(
    ...
    try {
      ...
      // Log error but don't throw
      debugPrint('Failed to trigger temperature alert: $e');
    }
  }
